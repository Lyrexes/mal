...
impl TokenStream {
    fn new(input: String) -> Result<TokenStream, String> {
        Ok(TokenStream { tokens: tokenize(input)? })
    }
...
fn read_form(tokens: &mut TokenStream) -> MalRet {
    let first_char = tokens.peek()
        .ok_or("EOF empty tokenstream".to_string())?
        .chars().next().ok_or("EOF empty token".to_string())?;
    match  first_char {
...
fn read_map(tokens: &mut TokenStream) -> MalRet {
    ...
    while let Some(token) = tokens.peek() {
        ...
        let key = match read_form(tokens)? {
            ...
            _ => return Err(format!("EOF: keys of the map must be a string or keyword!"))
        };
        if tokens.peek().is_none() {
            return Err(format!("EOF: no value for key: {}", key))
        }
...
fn read_sequence(tokens: &mut TokenStream, delimiter: &str) -> MalRet {
    ...
    match tokens.peek() {
        ...
        None => Err(format!("EOF: unbalanced sequence expected: {}", delimiter))
    }
}

fn read_atom(tokens: &mut TokenStream) -> MalRet{
    let mut token = tokens.next()
        .ok_or("EOF: expected a symbol".to_string())?;
...
fn read_string(tokens: &mut TokenStream) -> MalRet {
    ...
    if !token.ends_with('"') {
        return Err("EOF: invalid string literal: must end with '\"' ".to_owned());
    }
    ...
    Err("EOF: invalid string literal".to_owned())
}
...
mod tests {
    ...
    fn regex() {
        ...
        println!("{:?}", read_atom(&mut TokenStream::new("12345".to_string()).unwrap()));
        println!("{:?}", read_form(&mut TokenStream::new("\"12345\\n\\\"\"".to_string()).unwrap()));
        println!("{:?}", read_atom(&mut TokenStream::new(":sdklfs".to_string()).unwrap()));
        println!("{:?}", read_atom(&mut TokenStream::new("false".to_string()).unwrap()));
        println!("{:?}", read_atom(&mut TokenStream::new("true".to_string()).unwrap()));
        println!("{:?}", read_atom(&mut TokenStream::new("nil".to_string()).unwrap()));
        println!("{:?}", read_atom(&mut TokenStream::new("symbol".to_string()).unwrap()));
    }
}